...
source aca_deployment/containerapp-config.env
...
az acr build --registry $ACR_NAME -g $ACR_RG --image $BACKEND_NAME:1.0 --file aca_deployment/Dockerfile .


print_status "Deleting existing BACKEND Container App..."

az containerapp delete -n "$BACKEND_NAME" -g "$RESOURCE_GROUP" --yes

print_status "Deploying BACKEND Container App..."


echo "Creating container app: $BACKEND_NAME"
...
if [ $? -ne 0 ]
then
    printf "\nError deploying $BACKEND_NAME container app. Exiting...\n"
    exit 1
fi
